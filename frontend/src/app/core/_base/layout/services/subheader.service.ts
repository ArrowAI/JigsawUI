...
export class SubheaderService {
	// Public properties
	title$: BehaviorSubject<BreadcrumbTitle> = new BehaviorSubject<BreadcrumbTitle>({title: '', desc: '',showBack:false});
	breadcrumbs$: BehaviorSubject<Breadcrumb[]> = new BehaviorSubject<Breadcrumb[]>([]);
	actionButton$: BehaviorSubject<ActionButton[]> = new BehaviorSubject<ActionButton[]>([]);
	disabled$: Subject<boolean> = new Subject<boolean>();
	functionData$: BehaviorSubject<buttonFunction> = new BehaviorSubject<buttonFunction>({name:'',data:{}});
	// Private properties
	...
	private manualActionButton: any = {};
	 ...
	 */
     setActionButtons(actionButton:ActionButton[]|any[]){
		this.actionButton$.next(actionButton);
	 }
	 hideSubHeader(data:any){
	 ...
	 * @param title: string
	 */
	setTitle(title: string,showBack?:boolean) {
		this.manualTitle[this.router.url] = title;
		this.title$.next({title: title,showBack:showBack});
	}
	...
	callFunction(funcationData){
		this.functionData$.next(funcationData);
	}
