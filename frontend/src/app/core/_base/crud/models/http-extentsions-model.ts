...
export class HttpExtenstionsModel {
	...
	baseFilter(_entities: any[], _queryParams: QueryParamsModel, _filtrationFields: string[] = []): QueryResultsModel {
		...
		let __queryParams=_queryParams;
		delete __queryParams.filter.applicationId;
		let entitiesResult = this.searchInArray(_entities, __queryParams.filter, _filtrationFields);
